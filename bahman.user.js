...
(function () {
    ...
    function logNetwork(method, url, status, time, reqData, resData) {
        ...
        `;
        row.onclick = () => {
            console.group(`🔍 Details: ${cleanUrl}`);
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    ...
                    if (response.status === 401) {
                        log('❌ نشست منقضی شد. لطفا مجدد وارد شوید.');
    ...
    async function retryUntilSuccess(taskName, taskFn, delay = 1000, stopOnError = false) {
        while (isRunning) {
            try {
                ...
                log(`⏳ ${taskName}: در انتظار...`);
            } catch (e) {
                if (String(e).includes('401')) return null;
                log(`⚠️ خطا در ${taskName}: ${e}. تلاش مجدد...`);
    ...
    function renderLogin() {
        ...
        div.innerHTML = `
            ...
            <div class="bm-content">
                <input id="bm-user" class="bm-input" placeholder="کد ملی">
                ...
                <input id="bm-cap-input" class="bm-input" placeholder="کد امنیتی">
                <button id="bm-login-btn" class="bm-btn">ورود</button>
                <div id="bm-log-box" class="bm-log">لطفا وارد شوید...</div>
            </div>
        `;
        const refresh = async () => {
    ...
    function renderDashboard(user) {
        ...
        div.innerHTML = `
            ...
            <div class="bm-content">
                <div class="bm-row">
                    <div style="flex:1">
                        <div class="bm-label">نام خودرو</div>
                        <input id="cfg-car" class="bm-input" value="${cfg.targetCar}">
                    </div>
                    <div style="flex:1">
                        <div class="bm-label">فیلتر طرح (اختیاری)</div>
                        <input id="cfg-keywords" class="bm-input" value="${cfg.planKeywords}" placeholder="مثلا: نقدی، آذر">
                    </div>
                </div>
                <div class="bm-row">
                    <div style="flex:1">
                        <div class="bm-label">قیمت (اختیاری)</div>
                        <input id="cfg-price" class="bm-input" value="${cfg.targetPrice}" placeholder="0 = همه">
                    </div>
                     <div style="flex:1">
                        <div class="bm-label">اولویت رنگ</div>
                ...
                <div class="bm-row">
                    ...
                    <div style="flex:1">
                        <div class="bm-label">نام شهر</div>
                        <input id="cfg-cityname" class="bm-input" value="${cfg.cityName}" placeholder="مثلا: برازجان">
                    </div>
                </div>

                <button id="bm-start" class="bm-btn">شروع عملیات خرید</button>
        ...
        return new Promise(resolve => {
            ...
            overlay.innerHTML = `
                <div style="background:#1A1B26;padding:20px;border-radius:12px;width:320px;text-align:center;border:1px solid #00e676;box-shadow:0 0 20px rgba(0,230,118,0.2)">
                    ...
                    <div class="bm-status" id="cap-status" style="margin-bottom:5px;color:#aaa">حل خودکار...</div>
            ...
            solveCaptcha(imgUrl).then(text => {
                if (text) {
                    inp.value = text;
                    status.innerText = "✅ حل شد! ارسال...";
                    status.style.color = "#00e676";
                    setTimeout(submit, 300);
                } else {
                    status.innerText = "❌ حل نشد. دستی وارد کنید.";
    ...
    async function startBuyingFlow(userConfig) {
        try {
            ...
            log(`✅ محصول: ${product.title} (ID: ${product.id})`);
            ...
            while (isRunning && !orderId) {
                try {
                    ...
                    if (orderData.id && orderData.id !== "0") {
                        ...
                        if(!cities || !cities.length) throw new Error('شهری یافت نشد');
                        ...
                        log(`شهر: ${selectedCity ? selectedCity.title : 'تصادفی'}`);
                        ...
                        if(!branches || !branches.length) {
                            log('⚠️ نمایندگی یافت نشد. تلاش مجدد...');
                            throw new Error('نمایندگی موجود نیست');
                        }
                        ...
                        if(!fillData.queueId) throw new Error('خطا در صف');
                        log(`✅ در صف: ${fillData.queueId}`);

                        await checkResultLoop(fillData.queueId, orderId);
                        return;
                    } else {
                         if(orderData.errors) log(`⚠️ ${orderData.errors[0].errorDescription}`);
                         else log('⚠️ ثبت ناموفق');
                    }
